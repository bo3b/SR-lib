...
void SRContainer::createContext(Renderer& renderer, Window& window) {
    ...
    //! The DX11 Weaver may not be able to deal with textures which are larger than D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION (16384)
    if (weaverInputWidth > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION / 2 || weaverInputHeight > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION) {
        std::cout << "Warning: texture size in PredictingDX11Weaver may exceed the limits on this system." << std::endl;
    }
    //! [Get weaver input width and height]

    //! [Construct weaver]
    try {
        weaver = new SR::PredictingDX11Weaver(*context, renderer.GetDevice(), renderer.GetContext(), weaverInputWidth * 2, weaverInputHeight, window.GetHandle());
    }
    catch (const std::exception& e) {
        std::string errorMessage = e.what();
        std::cout << "Could not construct PredictingDX11Weaver: " << errorMessage << std::endl << "Aborting." << std::endl;
        exit(1);
    }
    //! [Construct weaver]
