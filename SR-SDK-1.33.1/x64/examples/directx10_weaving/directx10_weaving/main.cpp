...
ID3D10Buffer* TransformationBuffer = nullptr;
...
void RenderScene(Renderer& renderer, const int renderWidth, const int renderHeight, DirectX::XMFLOAT3 leftEye, DirectX::XMFLOAT3 rightEye, Pyramid& pyramid) {
    ...
    renderer.GetDevice()->UpdateSubresource(TransformationBuffer, 0, nullptr, &TransformationData, 0, 0);
    renderer.GetDevice()->VSSetConstantBuffers(0, 1, &TransformationBuffer);
    ...
    renderer.GetDevice()->RSSetViewports(1, &Viewport);
...
int main(void)
{
    ...
    //! [Construct weaver]
    weaver = new SR::PredictingDX10Weaver(*context, renderer.GetDevice(), renderWidth * 2, renderHeight, window.GetHandle());

    context->initialize();
    //! [Construct weaver]

    // Draw to weaver with application specific shaders
    ID3D10RenderTargetView* View = weaver->getFrameBuffer();
    ...
    do {
        ...
        renderer.GetDevice()->ClearRenderTargetView(View, clearColor);

        renderer.GetDevice()->OMSetRenderTargets(1, &View, nullptr);
