...
D3D10_VIEWPORT setUpViewPort(Window window) {
    D3D10_VIEWPORT Viewport;
    Viewport.TopLeftX = 0;
    Viewport.TopLeftY = 0;
...
D3D10_VIEWPORT renderImagePlane(ID3D10RenderTargetView* buffer, Renderer& renderer, TextureShader& shader, int renderWidth, int renderHeight, Window& window, ImagePlane& imagePlane) {
    ID3D10RenderTargetView* View = buffer;

    float Color[4] = { 0, 0, 0, 1 };
    renderer.GetDevice()->ClearRenderTargetView(View, Color);
    ...
    renderer.GetDevice()->OMSetRenderTargets(1, &View, nullptr);
    D3D10_VIEWPORT Viewport = setUpViewPort(window);
    renderer.GetDevice()->RSSetViewports(1, &Viewport);
...
{   
    ...
    do {
        ...
        if (container.isContextCreated()) {
            D3D10_VIEWPORT Viewport = renderImagePlane(container.weaver->getFrameBuffer(), renderer, shader, container.weaverInputWidth, container.weaverInputHeight, window, imagePlane);
