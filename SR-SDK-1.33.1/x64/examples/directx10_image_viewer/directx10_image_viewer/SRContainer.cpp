...
void SRContainer::createContext(Renderer& renderer, Window& window) {
    ...
    //! The DX10 Weaver may not be able to deal with textures which are larger than D3D10_REQ_TEXTURE2D_U_OR_V_DIMENSION (8192)
    if (weaverInputWidth > D3D10_REQ_TEXTURE2D_U_OR_V_DIMENSION / 2 || weaverInputHeight > D3D10_REQ_TEXTURE2D_U_OR_V_DIMENSION) {
        std::cout << "Warning: texture size in PredictingDX10Weaver may exceed the limits on this system." << std::endl;
    }
    //! [Get weaver input width and height]

    //! [Construct weaver]
    try {
        weaver = new SR::PredictingDX10Weaver(*context, renderer.GetDevice(), weaverInputWidth * 2, weaverInputHeight, window.GetHandle());
    }
    catch (const std::exception& e) {
        std::string errorMessage = e.what();
        std::cout << "Error: Could not construct PredictingDX10Weaver: " << errorMessage << std::endl << "Aborting." << std::endl;
        exit(1);
    }
    //! [Construct weaver]
