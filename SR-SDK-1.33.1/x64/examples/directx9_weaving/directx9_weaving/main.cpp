...
#pragma comment(lib, "dxguid.lib") // for IID_IDirect3DTexture9
...
public:
    ...
    virtual void accept(const SR_eyePair& eyePair) override
    {
        ...
        left = DirectX::XMFLOAT3((float)eyePair.left.x, (float)eyePair.left.y, (float)eyePair.left.z);
        right = DirectX::XMFLOAT3((float)eyePair.right.x, (float)eyePair.right.y, (float)eyePair.right.z);
    }
};

HWND WindowHandle;
// Create Window object
Window window("DirectXWindow", "Simulated Reality - Pyramid demo");
// Create SRContext
std::unique_ptr<SR::SRContext> context;
std::unique_ptr<SR::PredictingDX9Weaver> weaver;
std::mutex constructNewContextMutex;
std::unique_ptr<MyEyes> eyes;
std::unique_ptr<SystemEventMonitor> listener;

bool createSrContext() {
    context.reset();
    while (context == nullptr && contextValid == false) {
        try {
            context = std::make_unique<SR::SRContext>();
            return true;
        }
...
bool initializeSrObjects() {
    ...
    eyes = std::make_unique<MyEyes>(SR::EyeTracker::create(*context));

    SR::SystemSense* systemSense = SR::SystemSense::create(*context);
    listener = std::make_unique<SystemEventMonitor>();
    // set systemEvent listener to the newly constructed systemsense
    listener->stream.set(systemSense->openSystemEventStream(listener.get()));
...
void RenderScene(Renderer& renderer, const int renderWidth, const int renderHeight, DirectX::XMFLOAT3 leftEye, DirectX::XMFLOAT3 rightEye, Pyramid& pyramid) {
    //! [Drawing Scene]
...
int main(void)
{
    ...
    std::unique_ptr<ColorShader> shader = std::make_unique<ColorShader>(*renderer);
    std::unique_ptr<Pyramid> pyramid = std::make_unique<Pyramid>(*renderer);
    ...
    int systemResolutionWidth = GetSystemMetrics(SM_CXSCREEN);
    int systemResolutionHeight = GetSystemMetrics(SM_CYSCREEN);
    ...
    screenWidth_mm = systemResolutionWidth * 25.4f / dpiX;
    screenHeight_mm = systemResolutionHeight * 25.4f / dpiY;

    //! [Construct weaver]
    weaver = std::make_unique<SR::PredictingDX9Weaver>(*context, renderer->GetDevice(), renderWidth * 2, renderHeight, window.GetHandle());

    context->initialize();
    //! [Construct weaver]

    // Draw to weaver with application specific shaders
    IDirect3DSurface9* View = weaver->getFrameBuffer();

    do {
        window.Update();

        // IDirect3DDevice9::Present and IDirect3DDevice9::Reset methods return D3DERR_DEVICELOST error 
        // when the display cable of the monitor is detached. In this case the device object can not exit from the device lost state 
        // and thus it should be recreated along with the other attached resources.
        HRESULT hrCooperativeLevel = renderer->GetDevice()->TestCooperativeLevel();
        if (hrCooperativeLevel == D3DERR_DEVICELOST)
        {
            std::cout << "Device lost! Resetting the pipeline..." << std::endl;
            ...
            shader = std::make_unique<ColorShader>(*renderer);
            pyramid = std::make_unique<Pyramid>(*renderer);
            weaver = std::make_unique<SR::PredictingDX9Weaver>(*context, renderer->GetDevice(), renderWidth * 2, renderHeight, window.GetHandle());
            context->initialize();

            hrCooperativeLevel = renderer->GetDevice()->TestCooperativeLevel();
            if (FAILED(hrCooperativeLevel))
            {
                throw std::exception("Failed to reset the pipeline after tring to recover from the device lost state!");
            }
        }
        else if (renderer->GetSize().x != window.GetSize().x || renderer->GetSize().y != window.GetSize().y)
        {
            weaver->invalidateDeviceObjects();
            pyramid->invalidateDeviceObjects();
            shader->invalidateDeviceObjects();
            renderer->Resize(window.GetSize());
            shader->restoreDeviceObjects();
            pyramid->restoreDeviceObjects();
        ...
        // Clear framebuffer to grey
        renderer->GetDevice()->SetRenderTarget(0, View);
        const D3DCOLOR clearColor = D3DCOLOR(0xFF808080);
        ...
        if (contextValid && weaver != nullptr) {
            weaver->weave(Viewport.Width, Viewport.Height, Viewport.X, Viewport.Y);
        }
        //! [Render woven image]
